...
Public Class LocationPackagePaymentTermOverride

    'Add Dates
    Public Shared Function AddDates(ByVal iLocationPackageID As Integer, _
        ...
        If oReturn.Success Then
            ...
            ElseIf Not CheckNoOverlappingDates(iLocationPackageID, dStartDate, dEndDate) Then
                oReturn.AddWarning("The dates input conflict with existing dates for this ")
            ElseIf dStartDate < dStayStartDate OrElse dEndDate > dStayEndDate Then
                oReturn.AddWarning("The Payment Term Dates must be within the  Stay Dates")
            End If
        End If

        'do the bizzo
        If oReturn.Success Then
            oReturn.Result = SQL.Execute("exec LocationPackagePaymentTermOverrideAdd {0}, {1}, {2}", _
                iLocationPackageID, SQL.GetSqlValue(dStartDate, SQL.SqlValueType.Date), _
    ...
    Public Shared Function CheckNoOverlappingDates(ByVal iLocationPackageID As Integer, _
        ByVal dStartDate As Date, ByVal dEndDate As Date) As Boolean


        Return SQL.ExecuteSingleValue("exec LocationPackagePaymentTermOverrideCheckForOverlap {0},{1},{2}", _
            iLocationPackageID, SQL.GetSqlValue(dStartDate, SQL.SqlValueType.Date), _
            SQL.GetSqlValue(dEndDate, SQL.SqlValueType.Date)) = 0

    End Function

    Public Shared Sub Delete(ByVal iLocationPackagePaymentTermOverrideID As Integer)

        SQL.Execute("exec LocationPackagePaymentTermDelete {0}", iLocationPackagePaymentTermOverrideID)
